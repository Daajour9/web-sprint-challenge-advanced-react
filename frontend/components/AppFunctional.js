...
export default function AppFunctional(props) {
    ...
    function move(evt) {
        const direction = evt.target.id; // Get the button ID (left, up, right, down)
        const newIndex = getNextIndex(direction);
        if (newIndex !== index) {
            setIndex(newIndex);
            setSteps(steps + 1);
            setCoordinates(getXY(direction)); // Update message with new coordinates
    ...
    function onChange(evt) {
        ...
        setEmail(evt.target.value); // Update the email state
    }
